class MinigunTower extends Tower {
	constructor(){
		...
		this.price = [50, 25, 60, 100, 150]
		this.value = [25, 35, 65, 115, 190]
		...
		this.level_bonus = {'rof' : [0.01, 0.01, 0.02, 0.0],
				...
				'atk' : [0.25, 0.32, 0.45, 0],  //max 200%  total = 3x damage then level 1
				}
	}
	...
	get_tower_details_as_html(){
		...
		var icon_5 = this.level >= 5 ? "<li class='icon_dual_fire'>2</li>" : "";
		upgrade_price = this.level < 5 ? "$"+this.price[this.level] : "N/A";
		var contents = empty_str.concat("<h3>", header, " ", level, "<br /> (Sell $", this.value[this.level - 1], " Upgrade cost: ", upgrade_price ,")</h3>", description, "<br />", icon_1, icon_2, icon_3, icon_4, icon_5,"</ul>");
		console.log(header);
		return contents;
	}
				...
				var target_h = parseInt(opponent_gui_location.css("height"));
					...
					try {
						var t = setTimeout(() => {
						 ...
						 console.log("error", ferr); //The unit is already removed.
					}
