class CannonTower extends Tower {
	constructor(){
		...
		this.rof = 1000;
		this.atk = 50;
		this.price = [90, 30, 60, 100, 150]
		this.value = [45, 60, 90, 140, 210]
		this.towergraphic = "tower_cannon";
		this.weapon = "Orb_1";
		this.minimum_range = 50;
		this.max_splash_range = 30; 
		this.max_splash_damage = 30;
	...
	get_tower_details_as_html(){
		...
		var empty_str = "";
		var header = "Cannon Tower"
		var level = ""
		var bonus_damage = Math.round(cannontower_bonus_damage/100 * this.atk)
		var bonus_splash = Math.round(cannontower_bonus_splash_damage/100 * this.max_splash_damage)
		var s_damage = this.max_splash_damage + bonus_splash;
		var damage = this.atk + bonus_damage;
		for (var y = 0; y < this.level; y++) { level = level + "★"}
		level = "<span class='stars'>"+ level +"</span>"
		var sell_price = 0;
		var upgrade_price = 0;
		var description = "This slow firing throws a punch, can hit groups with massive damage. At level 5 adds stun to small enemies for a brief period. Medium range."
		...
		var icon_4 = "<li class='icon_atk'>"+this.atk+" (+"+bonus_damage+")</li>"
		var icon_5 = "<li class='icon_splash'>"+this.max_splash_range+"/"+s_damage+"</li>"
		var icon_6 = this.level >= 5 ? "<li class='icon_stun'>1</li>" : "";
	...
	attack_fire() {
			...
			try {
				...
				var fire_weapon = new Projectile();
				var damage = this.atk + Math.round(cannontower_bonus_damage/100 * this.atk);
				var splash_damage = this.max_splash_damage + Math.round(cannontower_bonus_splash_damage/100 * this.max_splash_damage)
				fire_weapon.create(opponent, this.is_projectile, this.weapon, true, bullet_start_x, bullet_start_y, bullet_end_x, bullet_end_y, this.rof, this.atk, splash_damage, this.max_splash_range, freeze_on_hit)
				fire_weapon.action();
